...
void ChatManager::switchToInterlocutor(const QString &name) {
    ...
    if (interlocutor != nullptr && interlocutorConfig != nullptr) {
        ...
        ModelInfo modelInfo =
            m_modelRegistry.findModel(interlocutorConfig->modelName());
        if (modelInfo.curationTriggerTokenCount >
            ...
            qWarning() << "Could not find valid model info for:"
                       << interlocutorConfig->modelName()
                       << "(Trigger:" << modelInfo.curationTriggerTokenCount
                       << "Target:" << modelInfo.curationTargetTokenCount << ")";
        }
    }
...
Interlocutor *
ChatManager::createInterlocutorFromConfig(InterlocutorConfig *config) {
