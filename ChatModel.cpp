...
void ChatModel::sendMessage(const QString &messageText)
{
    ...
    // Demander le compte de tokens des fichiers attachés pour l'exclure de la curation
    m_pendingFileTokenCount = 0;
    m_expectingFileTokenCount = !userAttachments.isEmpty();
    if (m_expectingFileTokenCount)
    {
        qDebug() << "Requesting token count for" << userAttachments.count() << "files.";
        m_interlocutor->countTokensForFiles(userAttachments);
    }
...
void ChatModel::handleNormalReply(const InterlocutorReply &reply)
{
    ...
    int currentInputTokens = reply.inputTokens;
    if (!m_expectingFileTokenCount)
    {
        currentInputTokens -= m_pendingFileTokenCount;
        if (currentInputTokens < 0)
            currentInputTokens = 0;
    }

    int newLiveMemorySize = currentInputTokens + reply.outputTokens;
    ...
    if (!reply.isIncomplete)
    {
        if (m_expectingFileTokenCount)
        {
            qDebug() << "Postponing curation check until file tokens count.";
        }
        else
        {
            checkCurationThreshold();
...
void ChatModel::setInterlocutor(Interlocutor *interlocutor)
{
    ...
    if (m_interlocutor)
    {
        ...
        connect(m_interlocutor, &Interlocutor::fileDeleted, this, &ChatModel::onFileDeleted,
                Qt::UniqueConnection);
        connect(m_interlocutor, &Interlocutor::fileTokenCountReady, this,
                &ChatModel::onFileTokenCountReady, Qt::UniqueConnection);
    }
}
...
void ChatModel::onFileTokenCountReady(int count)
{
    qDebug() << "File token count ready:" << count;
    m_pendingFileTokenCount = count;

    // Check if reply was already received (so we are in "postponed" state)
    // If m_expectingFileTokenCount is true AND we are NOT waiting for reply,
    // it means handleNormalReply() already ran and postponed the correction.
    if (m_expectingFileTokenCount && !m_isWaitingForReply)
    {
        qDebug() << "Reply came first. Correcting live memory and checking curation.";

        m_liveMemoryTokens -= m_pendingFileTokenCount;
        if (m_liveMemoryTokens < 0)
            m_liveMemoryTokens = 0; // Safety
        emit liveMemoryTokensChanged();
        qDebug() << "Live Memory size corrected to:" << m_liveMemoryTokens;

        checkCurationThreshold();
    }
    // We have received the count, so we are no longer expecting it.
    // If reply hasn't come yet, handleNormalReply will see expecting=false
    // and simply subtract m_pendingFileTokenCount.
    m_expectingFileTokenCount = false;
}
